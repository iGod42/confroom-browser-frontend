...
const ConferenceRoomStatus = ({ socketUrl }: { socketUrl: string }) => {
	...
	const nextEvent = EventTools.getTodaysFutureEvents(currentTime, events, { includeCurrent: false })[0]
	const timeToBook = Math.ceil(Math.min((nextEvent ? (nextEvent.start.getTime() - currentTime.getTime()) / 60000 : 999), 15))
			...
			<React.Fragment>
				...
				{isLoading ? null : !currentEvent ?
					<ActionButton onClick={bookRoom} color="secondary" variant={"extended"} size="large"><i
						className="material-icons">meeting_room</i>Book {timeToBook} min</ActionButton>
